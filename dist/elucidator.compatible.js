...
*/var FableCoreServiceProviderBase=/*#__PURE__*/function(){function FableCoreServiceProviderBase(pOptions,pServiceHash){_classCallCheck(this,FableCoreServiceProviderBase);this.fable=false;this.options=_typeof(pOptions)==='object'?pOptions:{};this.serviceType='Unknown';// The hash will be a non-standard UUID ... the UUID service uses this base class!
this.UUID="CORESVC-".concat(Math.floor(Math.random()*(99999-10000)+10000));this.Hash=typeof pServiceHash==='string'?pServiceHash:"".concat(this.UUID);}_createClass(FableCoreServiceProviderBase,[{key:"connectFable",value:// After fable is initialized, it would be expected to be wired in as a normal service.
function connectFable(pFable){this.fable=pFable;return true;}}]);return FableCoreServiceProviderBase;}();_defineProperty(FableCoreServiceProviderBase,"isFableService",true);module.exports=FableCoreServiceProviderBase;},{}],3:[function(require,module,exports){/**
* Fable Service Base
* @author <steven@velozo.com>
*/var FableServiceProviderBase=/*#__PURE__*/_createClass(function FableServiceProviderBase(pFable,pOptions,pServiceHash){_classCallCheck(this,FableServiceProviderBase);this.fable=pFable;this.options=_typeof(pOptions)==='object'?pOptions:_typeof(pFable)==='object'&&!pFable.isFable?pFable:{};this.serviceType='Unknown';if(typeof pFable.getUUID=='function'){this.UUID=pFable.getUUID();}else{this.UUID="NoFABLESVC-".concat(Math.floor(Math.random()*(99999-10000)+10000));}this.Hash=typeof pServiceHash==='string'?pServiceHash:"".concat(this.UUID);// Pull back a few things
this.log=this.fable.log;this.servicesMap=this.fable.serviceMap;this.services=this.fable.services;});_defineProperty(FableServiceProviderBase,"isFableService",true);module.exports=FableServiceProviderBase;module.exports.CoreServiceProviderBase=require('./Fable-ServiceProviderBase-Preinit.js');},{"./Fable-ServiceProviderBase-Preinit.js":2}],4:[function(require,module,exports){/**
* @license MIT
* @author <steven@velozo.com>
*/ /**
...
},{key:"addInstruction",value:function addInstruction(pInstructionHash,fInstructionFunction){if(typeof pInstructionHash!='string'){this.elucidator.log.error("Attempted to add an instruction with an invalid hash; expected a string but the instruction hash type was ".concat(_typeof(pInstructionHash)));return false;}if(typeof fInstructionFunction!='function'){this.elucidator.log.error("Attempted to add an instruction with an invalid function; expected a function but type was ".concat(_typeof(fInstructionFunction)));return false;}this.elucidator.instructionSets[this.namespace.toLowerCase()][pInstructionHash]=fInstructionFunction;return true;}},{key:"initializeInstructions",value:function initializeInstructions(){// This is where we map in the instructions.
...
this.addInstruction('noop',function(pOperation){pOperation.log.info('Executing a no-operation operation.');return true;});return true;}// Add an operation to the set
},{key:"addOperation",value:function addOperation(pOperationHash,pOperation){if(typeof pOperationHash!='string'){this.elucidator.log.error("Attempted to add an operation with an invalid hash; expected a string but the operation hash type was ".concat(_typeof(pOperationHash)),pOperation);return false;}if(_typeof(pOperation)!='object'){this.elucidator.log.error("Attempted to add an invalid operation; expected an object data type but the type was ".concat(_typeof(pOperation)),pOperation);return false;}// Validate the Description subobject, which is key to functioning.
if(!pOperation.hasOwnProperty("Description")){this.elucidator.log.error("Attempted to add an operation with an invalid description; no Description subobject set.",pOperation);return false;}if(_typeof(pOperation.Description)!='object'){this.elucidator.log.error("Attempted to add an operation with an invalid description; Description subobject was not an object.  The type was ".concat(_typeof(pOperation.Description),"."),pOperation);return false;}if(typeof pOperation.Description.Hash!='string'){if(typeof pOperation.Description.Operation=='string'){// Use the "Operation" as the "Hash"
pOperation.Description.Hash=pOperation.Description.Operation;}else{this.elucidator.log.error("Attempted to add an operation with an invalid description; Description subobject did not contain a valid Hash which is required to call the operation.",pOperation);return false;}}// Now auto create data if it is missing or wrong in the Description
...
if(_typeof(pOperation.Inputs)!=='object'){this.elucidator.log.error("Attempted to add an operation with an invalid Inputs object.",pOperation);return false;}// If there are no inputs, or outputs, or steps, add them.
// TODO: Add a step where we try to load this into Manyfest and see that it's valid.
if(_typeof(pOperation.Outputs)!=='object'){this.elucidator.log.error("Attempted to add an operation with an invalid Outputs object.",pOperation);return false;}if(!Array.isArray(pOperation.Steps)){this.elucidator.log.error("Attempted to add an operation with an invalid Steps array.",pOperation);return false;}this.elucidator.operationSets[this.namespace.toLowerCase()][pOperationHash.toLowerCase()]=pOperation;return true;}},{key:"initializeOperations",value:function initializeOperations(){this.addOperation('noop',{"Description":{"Operation":"noop","Description":"No operation - no affect on any data."}});return true;}}]);return ElucidatorInstructionSet;}();;module.exports=ElucidatorInstructionSet;},{}],5:[function(require,module,exports){/**
* @license MIT
* @author <steven@velozo.com>
*/var libFableServiceProviderBase=require('fable-serviceproviderbase');var libElucidatorInstructionSet=require('./Elucidator-InstructionSet.js');/**
* Elucidator object address-based descriptions and manipulations.
*
* @class Elucidator
*/var Elucidator=/*#__PURE__*/function(_libFableServiceProvi){_inherits(Elucidator,_libFableServiceProvi);var _super=_createSuper(Elucidator);function Elucidator(pFable,pOptions,pServiceHash){var _this;_classCallCheck(this,Elucidator);_this=_super.call(this,pFable,pOptions,pServiceHash);// Instructions are the basic building blocks for operations
_this.instructionSets={};// Operations are the solvers that can be called (instructions can't be called directly)
// These can be added at run-time as well
_this.operationSets={};// Decide later how to make this truly unique.
_this.UUID=0;_this.loadDefaultInstructionSets();if(_this.options.OperationSet){var tmpSolverHashes=Object.keys(_this.options.OperationSet);for(var i=0;i<tmpSolverHashes.length;i++){_this.addOperation('Custom',tmpSolverHashes[i],_this.options.OperationSet[tmpSolverHashes[i]]);}}return _this;}// Load an instruction set
...
tmpDescriptionManyfest=this.fable.serviceManager.instantiateServiceProviderWithoutRegistration('Manifest');// Synthesize a manyfest from the Input and Output properties
...
tmpOperation.UUID=this.UUID++;tmpOperation.SolutionContext=tmpSolutionContext;if(tmpOperation.Description.Synopsys){tmpSolutionContext.SolutionLog.push("[".concat(tmpOperation.UUID,"]: Solver running operation ").concat(tmpOperation.Description.Synopsys));}var tmpPrecedent=this.fable.serviceManager.instantiateServiceProviderWithoutRegistration('MetaTemplate');tmpPrecedent.addPattern('{{Name:','}}',function(pHash){var tmpHash=pHash.trim();var tmpDescriptor=tmpDescriptionManyfest.getDescriptorByHash(tmpHash);// Return a human readable value
...
var _loop=function _loop(_i){var tmpStep=tmpOperation.Steps[_i];// Instructions are always endpoints -- they *do not* recurse.
...
tmpDescriptionManyfest.schemaManipulations.resolveAddressMappings(tmpInputSchema.Descriptors,tmpStep.InputHashAddressMap);var tmpInputManyfest=_this2.fable.serviceManager.instantiateServiceProviderWithoutRegistration('Manifest',tmpInputSchema);if(tmpSolutionContext.InputHashMapping){tmpInputManyfest.hashTranslations.addTranslation(tmpSolutionContext.InputHashMapping);}var tmpOutputSchema={"Scope":"OutputObject","Descriptors":JSON.parse(JSON.stringify(tmpOperation.Outputs))};tmpDescriptionManyfest.schemaManipulations.resolveAddressMappings(tmpOutputSchema.Descriptors,tmpStep.OutputHashAddressMap);var tmpOutputManyfest=_this2.fable.serviceManager.instantiateServiceProviderWithoutRegistration('Manifest',tmpOutputSchema);if(tmpSolutionContext.OutputHashMapping){tmpOutputManyfest.hashTranslations.addTranslation(tmpSolutionContext.OutputHashMapping);}// Construct the instruction state object
var tmpInstructionState={Elucidator:_this2,Namespace:tmpStep.Namespace.toLowerCase(),Instruction:tmpStep.Instruction.toLowerCase(),Operation:tmpOperation,SolutionContext:tmpSolutionContext,DescriptionManyfest:tmpDescriptionManyfest,InputObject:tmpInputObject,InputManyfest:tmpInputManyfest,OutputObject:tmpOutputObject,OutputManyfest:tmpOutputManyfest};tmpInstructionState.logError=function(pMessage){var tmpErrorMessage="[Operation ".concat(tmpInstructionState.Operation.Description.Namespace,":").concat(tmpInstructionState.Operation.Description.Hash," - Step #").concat(_i,":").concat(tmpStep.Namespace,":").concat(tmpStep.Instruction,"] ").concat(pMessage);_this2.log.error(tmpErrorMessage);tmpSolutionContext.SolutionLog.push("[ERROR]".concat(tmpErrorMessage));};tmpInstructionState.logInfo=function(pMessage){var tmpInfoMessage="[Operation ".concat(tmpInstructionState.Operation.Description.Namespace,":").concat(tmpInstructionState.Operation.Description.Hash," - Step #").concat(_i,":").concat(tmpStep.Namespace,":").concat(tmpStep.Instruction,"] ").concat(pMessage);tmpSolutionContext.SolutionLog.push("[INFO]".concat(tmpInfoMessage));};if(_this2.instructionSets[tmpInstructionState.Namespace].hasOwnProperty(tmpInstructionState.Instruction)){var fInstruction=_this2.instructionSets[tmpInstructionState.Namespace][tmpInstructionState.Instruction];fInstruction(tmpInstructionState);}}// Operations recurse.
if(tmpStep.hasOwnProperty('Operation')){if(typeof tmpStep.Operation=='string'){_this2.solveInternalOperation(tmpStep.Namespace,tmpStep.Operation,tmpInputObject,tmpOutputObject,tmpDescriptionManyfest,tmpStep.InputHashAddressMap,tmpStep.OutputHashAddressMap,tmpSolutionContext);}else if(_typeof(tmpStep.Operation)=='object'){// You can even define an inline object operation!  This gets crazy fast
_this2.solveOperation(tmpStep.Operation,tmpInputObject,tmpOutputObject,tmpDescriptionManyfest,tmpStep.InputHashAddressMap,tmpStep.OutputHashAddressMap,tmpSolutionContext);}}};for(var _i=0;_i<tmpOperation.Steps.length;_i++){_loop(_i);}if(tmpOperation.hasOwnProperty('Log')&&tmpOperation.Log.hasOwnProperty('PostOperation')){if(typeof tmpOperation.Log.PostOperation=='string'){tmpOperation.SolutionContext.SolutionLog.push(tmpPrecedent.parseString(tmpOperation.Log.PostOperation));}else if(Array.isArray(tmpOperation.Log.PreOperation)){for(var _i2=0;_i2<tmpOperation.Log.PostOperation.length;_i2++){if(typeof tmpOperation.Log.PostOperation[_i2]=='string'){tmpOperation.SolutionContext.SolutionLog.push(tmpPrecedent.parseString(tmpOperation.Log.PostOperation[_i2]));}}}}return tmpSolutionContext;}}]);return Elucidator;}(libFableServiceProviderBase);;module.exports=Elucidator;},{"./Elucidator-InstructionSet.js":4,"./InstructionSets/Geometry.js":6,"./InstructionSets/Logic.js":7,"./InstructionSets/Math-Javascript.js":8,"./InstructionSets/PreciseMath-Decimal.js":32,"./InstructionSets/Set.js":33,"./InstructionSets/String.js":34,"fable-serviceproviderbase":3}],6:[function(require,module,exports){// Solution providers are meant to be stateless, and not classes.
// These solution providers are akin to drivers, connecting code libraries or 
// other types of behavior to mapping operations.
var libElucidatorInstructionSet=require('../Elucidator-InstructionSet.js');var Geometry=/*#__PURE__*/function(_libElucidatorInstruc){_inherits(Geometry,_libElucidatorInstruc);var _super2=_createSuper(Geometry);function Geometry(pElucidator){var _this3;_classCallCheck(this,Geometry);_this3=_super2.call(this,pElucidator);_this3.namespace='Geometry';return _this3;}// Geometry provides no instructions
_createClass(Geometry,[{key:"initializeInstructions",value:function initializeInstructions(){return true;}},{key:"initializeOperations",value:function initializeOperations(){this.addOperation('rectanglearea',require("./Operations/Geometry-RectangleArea.json"));return true;}}]);return Geometry;}(libElucidatorInstructionSet);module.exports=Geometry;},{"../Elucidator-InstructionSet.js":4,"./Operations/Geometry-RectangleArea.json":9}],7:[function(require,module,exports){// Solution providers are meant to be stateless, and not classes.
// These solution providers are akin to drivers, connecting code libraries or 
// other types of behavior to mapping operations.
...
pOperation.logInfo("Creating a new group [".concat(tmpGroupByValue,"] for element [").concat(i,"]."));tmpOutputDataSet[tmpGroupByValue]=[];}tmpOutputDataSet[tmpGroupByValue].push(tmpValue[tmpGroupValueProperty]);}}}}else{var tmpObjectKeys=Object.keys(tmpInputDataSet);for(var _i6=0;_i6<tmpObjectKeys.length;_i6++){if(_typeof(tmpInputDataSet[tmpObjectKeys[_i6]])!=='object'){pOperation.logInfo("Element [".concat(_i6,"] was not an object; skipping group operation."));}else{var _tmpValue6=tmpInputDataSet[tmpObjectKeys[_i6]];var _tmpGroupByValue2=_tmpValue6[tmpGroupByProperty];if(!_tmpValue6.hasOwnProperty(tmpGroupByProperty)){pOperation.logInfo("Element [".concat(tmpObjectKeys[_i6],"][").concat(_i6,"] doesn't have the group by property [").concat(tmpGroupByProperty,"]; setting group to [__NO_GROUP]."));_tmpGroupByValue2='__NO_GROUP';}if(!_tmpValue6.hasOwnProperty(tmpGroupValueProperty)){pOperation.logInfo("Element [".concat(tmpObjectKeys[_i6],"][").concat(_i6,"] doesn't have the group value property [").concat(tmpGroupValueProperty,"]; skipping group operation."));}else{if(!tmpOutputDataSet.hasOwnProperty(_tmpGroupByValue2)){// Create a new grouped value
pOperation.logInfo("Creating a new group [".concat(_tmpGroupByValue2,"] for element [").concat(tmpObjectKeys[_i6],"][").concat(_i6,"]."));tmpOutputDataSet[_tmpGroupByValue2]=[];}tmpOutputDataSet[_tmpGroupByValue2].push(_tmpValue6[tmpGroupValueProperty]);}}}}}else{pOperation.logError("Input set is neither an Array nor an Object");}pOperation.OutputManyfest.setValueByHash(pOperation.OutputObject,'outputDataSet',tmpOutputDataSet);return true;};var Set=/*#__PURE__*/function(_libElucidatorInstruc5){_inherits(Set,_libElucidatorInstruc5);var _super6=_createSuper(Set);function Set(pElucidator){var _this7;_classCallCheck(this,Set);_this7=_super6.call(this,pElucidator);_this7.namespace='Set';return _this7;}_createClass(Set,[{key:"initializeInstructions",value:function initializeInstructions(){// Logic actually wants a noop instruction!
_get(_getPrototypeOf(Set.prototype),"initializeInstructions",this).call(this);this.addInstruction('groupvaluesby',groupValuesBy);return true;}},{key:"initializeOperations",value:function initializeOperations(){this.addOperation('groupvaluesby',require("./Operations/Set-GroupValuesBy.json"));return true;}}]);return Set;}(libElucidatorInstructionSet);module.exports=Set;},{"../Elucidator-InstructionSet.js":4,"./Operations/Set-GroupValuesBy.json":28}],34:[function(require,module,exports){// Solution providers are meant to be stateless, and not classes.
// These solution providers are akin to drivers, connecting code libraries or 
// other types of behavior to mapping operations.
var libElucidatorInstructionSet=require('../Elucidator-InstructionSet.js');var trim=function trim(pOperation){var tmpInputString=pOperation.InputManyfest.getValueByHash(pOperation.InputObject,'inputString');pOperation.OutputManyfest.setValueByHash(pOperation.OutputObject,'outputString',tmpInputString.trim());return true;};var replace=function replace(pOperation){var tmpInputString=pOperation.InputManyfest.getValueByHash(pOperation.InputObject,'inputString');var tmpSearchFor=pOperation.InputManyfest.getValueByHash(pOperation.InputObject,'searchFor');var tmpReplaceWith=pOperation.InputManyfest.getValueByHash(pOperation.InputObject,'replaceWith');pOperation.OutputManyfest.setValueByHash(pOperation.OutputObject,'outputString',tmpInputString.replace(tmpSearchFor,tmpReplaceWith));return true;};var substring=function substring(pOperation){var tmpInputString=pOperation.InputManyfest.getValueByHash(pOperation.InputObject,'inputString');var indexStart=pOperation.InputManyfest.getValueByHash(pOperation.InputObject,'indexStart');var indexEnd=pOperation.InputManyfest.getValueByHash(pOperation.InputObject,'indexEnd');if(indexEnd!=null){pOperation.OutputManyfest.setValueByHash(pOperation.OutputObject,'outputString',tmpInputString.substring(indexStart,indexEnd));}else{pOperation.OutputManyfest.setValueByHash(pOperation.OutputObject,'outputString',tmpInputString.substring(indexStart));}return true;};var StringOperations=/*#__PURE__*/function(_libElucidatorInstruc6){_inherits(StringOperations,_libElucidatorInstruc6);var _super7=_createSuper(StringOperations);function StringOperations(pElucidator){var _this8;_classCallCheck(this,StringOperations);_this8=_super7.call(this,pElucidator);_this8.namespace='String';return _this8;}_createClass(StringOperations,[{key:"initializeInstructions",value:function initializeInstructions(){this.addInstruction('trim',trim);this.addInstruction('replace',replace);this.addInstruction('substring',substring);return true;}},{key:"initializeOperations",value:function initializeOperations(){this.addOperation('trim',require("./Operations/String-Trim.json"));this.addOperation('replace',require("./Operations/String-Replace.json"));this.addOperation('substring',require("./Operations/String-Substring.json"));return true;}}]);return StringOperations;}(libElucidatorInstructionSet);module.exports=StringOperations;},{"../Elucidator-InstructionSet.js":4,"./Operations/String-Replace.json":29,"./Operations/String-Substring.json":30,"./Operations/String-Trim.json":31}]},{},[5])(5);});