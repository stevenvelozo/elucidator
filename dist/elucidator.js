...
connectFable(pFable){this.fable=pFable;return true;}}_defineProperty(FableCoreServiceProviderBase,"isFableService",true);module.exports=FableCoreServiceProviderBase;},{}],3:[function(require,module,exports){/**
* Fable Service Base
* @author <steven@velozo.com>
...
addInstruction(pInstructionHash,fInstructionFunction){if(typeof pInstructionHash!='string'){this.elucidator.log.error("Attempted to add an instruction with an invalid hash; expected a string but the instruction hash type was ".concat(typeof pInstructionHash));return false;}if(typeof fInstructionFunction!='function'){this.elucidator.log.error("Attempted to add an instruction with an invalid function; expected a function but type was ".concat(typeof fInstructionFunction));return false;}this.elucidator.instructionSets[this.namespace.toLowerCase()][pInstructionHash]=fInstructionFunction;return true;}initializeInstructions(){// This is where we map in the instructions.
...
this.addInstruction('noop',pOperation=>{pOperation.log.info('Executing a no-operation operation.');return true;});return true;}// Add an operation to the set
addOperation(pOperationHash,pOperation){if(typeof pOperationHash!='string'){this.elucidator.log.error("Attempted to add an operation with an invalid hash; expected a string but the operation hash type was ".concat(typeof pOperationHash),pOperation);return false;}if(typeof pOperation!='object'){this.elucidator.log.error("Attempted to add an invalid operation; expected an object data type but the type was ".concat(typeof pOperation),pOperation);return false;}// Validate the Description subobject, which is key to functioning.
if(!pOperation.hasOwnProperty("Description")){this.elucidator.log.error("Attempted to add an operation with an invalid description; no Description subobject set.",pOperation);return false;}if(typeof pOperation.Description!='object'){this.elucidator.log.error("Attempted to add an operation with an invalid description; Description subobject was not an object.  The type was ".concat(typeof pOperation.Description,"."),pOperation);return false;}if(typeof pOperation.Description.Hash!='string'){if(typeof pOperation.Description.Operation=='string'){// Use the "Operation" as the "Hash"
...
if(typeof pOperation.Inputs!=='object'){this.elucidator.log.error("Attempted to add an operation with an invalid Inputs object.",pOperation);return false;}// If there are no inputs, or outputs, or steps, add them.
// TODO: Add a step where we try to load this into Manyfest and see that it's valid.
if(typeof pOperation.Outputs!=='object'){this.elucidator.log.error("Attempted to add an operation with an invalid Outputs object.",pOperation);return false;}if(!Array.isArray(pOperation.Steps)){this.elucidator.log.error("Attempted to add an operation with an invalid Steps array.",pOperation);return false;}this.elucidator.operationSets[this.namespace.toLowerCase()][pOperationHash.toLowerCase()]=pOperation;return true;}initializeOperations(){this.addOperation('noop',{"Description":{"Operation":"noop","Description":"No operation - no affect on any data."}});return true;}};module.exports=ElucidatorInstructionSet;},{}],5:[function(require,module,exports){/**
* @license MIT
* @author <steven@velozo.com>
*/const libFableServiceProviderBase=require('fable-serviceproviderbase');const libElucidatorInstructionSet=require('./Elucidator-InstructionSet.js');/**
* Elucidator object address-based descriptions and manipulations.
*
* @class Elucidator
*/class Elucidator extends libFableServiceProviderBase{constructor(pFable,pOptions,pServiceHash){super(pFable,pOptions,pServiceHash);// Instructions are the basic building blocks for operations
...
this.UUID=0;this.loadDefaultInstructionSets();if(this.options.OperationSet){let tmpSolverHashes=Object.keys(this.options.OperationSet);for(let i=0;i<tmpSolverHashes.length;i++){this.addOperation('Custom',tmpSolverHashes[i],this.options.OperationSet[tmpSolverHashes[i]]);}}}// Load an instruction set
...
tmpDescriptionManyfest.schemaManipulations.resolveAddressMappings(tmpInputSchema.Descriptors,tmpStep.InputHashAddressMap);let tmpInputManyfest=this.fable.serviceManager.instantiateServiceProviderWithoutRegistration('Manifest',tmpInputSchema);if(tmpSolutionContext.InputHashMapping){tmpInputManyfest.hashTranslations.addTranslation(tmpSolutionContext.InputHashMapping);}let tmpOutputSchema={"Scope":"OutputObject","Descriptors":JSON.parse(JSON.stringify(tmpOperation.Outputs))};tmpDescriptionManyfest.schemaManipulations.resolveAddressMappings(tmpOutputSchema.Descriptors,tmpStep.OutputHashAddressMap);let tmpOutputManyfest=this.fable.serviceManager.instantiateServiceProviderWithoutRegistration('Manifest',tmpOutputSchema);if(tmpSolutionContext.OutputHashMapping){tmpOutputManyfest.hashTranslations.addTranslation(tmpSolutionContext.OutputHashMapping);}// Construct the instruction state object
let tmpInstructionState={Elucidator:this,Namespace:tmpStep.Namespace.toLowerCase(),Instruction:tmpStep.Instruction.toLowerCase(),Operation:tmpOperation,SolutionContext:tmpSolutionContext,DescriptionManyfest:tmpDescriptionManyfest,InputObject:tmpInputObject,InputManyfest:tmpInputManyfest,OutputObject:tmpOutputObject,OutputManyfest:tmpOutputManyfest};tmpInstructionState.logError=pMessage=>{let tmpErrorMessage="[Operation ".concat(tmpInstructionState.Operation.Description.Namespace,":").concat(tmpInstructionState.Operation.Description.Hash," - Step #").concat(i,":").concat(tmpStep.Namespace,":").concat(tmpStep.Instruction,"] ").concat(pMessage);this.log.error(tmpErrorMessage);tmpSolutionContext.SolutionLog.push("[ERROR]".concat(tmpErrorMessage));};tmpInstructionState.logInfo=pMessage=>{let tmpInfoMessage="[Operation ".concat(tmpInstructionState.Operation.Description.Namespace,":").concat(tmpInstructionState.Operation.Description.Hash," - Step #").concat(i,":").concat(tmpStep.Namespace,":").concat(tmpStep.Instruction,"] ").concat(pMessage);tmpSolutionContext.SolutionLog.push("[INFO]".concat(tmpInfoMessage));};if(this.instructionSets[tmpInstructionState.Namespace].hasOwnProperty(tmpInstructionState.Instruction)){let fInstruction=this.instructionSets[tmpInstructionState.Namespace][tmpInstructionState.Instruction];fInstruction(tmpInstructionState);}}// Operations recurse.
...
initializeInstructions(){return true;}initializeOperations(){this.addOperation('rectanglearea',require("./Operations/Geometry-RectangleArea.json"));return true;}}module.exports=Geometry;},{"../Elucidator-InstructionSet.js":4,"./Operations/Geometry-RectangleArea.json":9}],7:[function(require,module,exports){// Solution providers are meant to be stateless, and not classes.
// These solution providers are akin to drivers, connecting code libraries or 
// other types of behavior to mapping operations.
...
super.initializeInstructions();this.addInstruction('if',ifInstruction);this.addInstruction('execute',executeOperation);return true;}initializeOperations(){this.addOperation('if',require("./Operations/Logic-If.json"));this.addOperation('execute',require("./Operations/Logic-Execute.json"));return true;}}module.exports=Logic;},{"../Elucidator-InstructionSet.js":4,"./Operations/Logic-Execute.json":10,"./Operations/Logic-If.json":11}],8:[function(require,module,exports){// Solution providers are meant to be stateless, and not classes.
// These solution providers are akin to drivers, connecting code libraries or 
// other types of behavior to mapping operations.
...
super.initializeInstructions();this.addInstruction('groupvaluesby',groupValuesBy);return true;}initializeOperations(){this.addOperation('groupvaluesby',require("./Operations/Set-GroupValuesBy.json"));return true;}}module.exports=Set;},{"../Elucidator-InstructionSet.js":4,"./Operations/Set-GroupValuesBy.json":28}],34:[function(require,module,exports){// Solution providers are meant to be stateless, and not classes.
// These solution providers are akin to drivers, connecting code libraries or 
// other types of behavior to mapping operations.
